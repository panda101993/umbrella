  ...
  otpSubmit(){
    if (this.state.phoneOTP1Status && this.state.phoneOTP2Status && this.state.phoneOTP3Status && this.state.phoneOTP4Status
        ...
        submitOTPApi() {
          ...
          ApiRequest(OtpDetails, `user-management/verify-otp/${this.props.userdetails.uuid}`, "POST")
            .then(resp => {
              console.log("otpverifyres.data.token",resp)
              switch (resp.status) {
                case (900): {
        ...
        ResendApi() {
            this.setState({isLoading:true})
          ApiRequest( "",`user-management/resend-otp/${this.props.userdetails.uuid}`, "POST")
            .then(resp => {
...
const mapStateToProps = state => {
  console.log("State_on_Login_Page==>>otpppp ", state)
  return {
